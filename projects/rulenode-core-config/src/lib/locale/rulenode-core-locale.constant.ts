    ...
    const enUS = {
      tb: {
        rulenode: {
          ...
          'update-attributes-on-value-change':'Save attribute only if the value changes',
          'update-attributes-on-value-change-hint': 'Updates the attribute on every incoming message disregarding if its value has changed. Increases API usage and reduces performance.',
          'update-attributes-on-value-change-hint-enabled': 'Updates the attribute only if its value has changed. If the value is not changed, no update to the attribute timestamp nor attribute change notification will be sent.',
